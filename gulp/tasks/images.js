...
export const images = () => {
    return app.gulp.src(app.path.src.images)
        ...
        .pipe(app.gulp.dest(app.path.build.images))      // выгружаем webp-изображения в папку с результатами
        .pipe(app.gulp.src(app.path.src.images))         // получение доступа к изображениям в папке исходников
        .pipe(app.plugins.newer(app.path.build.images))  // снова проверка изображений в папке результатов
        .pipe(imagemin({       // оптимизация (сжимание) изображений
            progressive: true,
